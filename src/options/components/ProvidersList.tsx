      ...
      return (
        <Table.Row key={providerSetting.providerCanonicalName}>
          ...
          <Table.Cell textAlign="left" width="8">
            <Checkbox
              toggle
              onChange={onToggle(props)}
              value={providerSetting.providerCanonicalName}
        ...
        <Grid.Column>
          <Segment padded="very" stacked>
            <Header as="h3" textAlign="center">
              <Icon name="toggle on" />
              <Header.Content>Toggle Analytics Providers</Header.Content>
            </Header>
            <Table size="large" columns="2" color="blue">
              <Table.Body>{renderRows(this.props)}</Table.Body>
            </Table>
            <Message warning>
              <Icon name="trademark" />
              <Icon name="copyright" />
              All product names, logos, and brands are property of their respective owners. All company, product, and
              service names used in this browser extension are for identification purposes only. Use of these names,
